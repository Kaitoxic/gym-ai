...
router.post('/ask', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const profileCtx = profile
      ? `Profil de l'utilisateur : objectif=${profile.goal ?? 'non défini'}, niveau=${profile.fitness_level ?? 'non défini'}, poids=${profile.body_weight ?? '?'}kg, taille=${profile.body_height ?? '?'}cm, âge=${profile.body_age ?? '?'} ans, jours d'entraînement par semaine=${(profile.available_days ?? []).length}, équipement=[${(profile.equipment ?? []).join(', ')}].`
      : '';
    ...
    const systemPrompt = `Tu es un coach sportif expert, polyvalent, maîtrisant la musculation, le cardio et la nutrition sportive. Tu donnes des conseils précis, basés sur des preuves scientifiques, adaptés à la pratique de la musculation et du powerbuilding. Tes réponses sont en français.
